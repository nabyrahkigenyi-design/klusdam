...
export default function Projects() {
  ...
  // Touch drag to pan + double-tap to toggle zoom
  function onTouchStart(e: React.TouchEvent) {
    ...
    if (now - lastTap.current < 300) {
      ...
      // start drag only when zoomed in; otherwise allow page scroll
      const t = e.touches[0];
      dragging.current = true;
  ...
  // Wheel zoom: allow page to scroll when zoom === 1, only block when actually zooming
  function onWheel(e: React.WheelEvent) {
    if (!open || zoom === 1) return; // do nothing: page can scroll normally
    e.preventDefault(); // block page scroll while adjusting zoom at zoom>1
      ...
      {open && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center">
          ...
          <button
            ...
            onClick={(e) => {
              // close only if they clicked the dark backdrop, not the image box
